...
Currently, the only graph we have is a Heuristic Graph, and an undirected weighted graph.

For more traditional graphs, we could also implement undirected weighted graphs. There is no use for unweighted graphs because there cannot exist a weight functionality, but we could add it s.t, behind the scenes, all the weights are the same.

We also do not have directed graphs, which we could add by not adding the `from node' to the `to node''s adjacency list. 
