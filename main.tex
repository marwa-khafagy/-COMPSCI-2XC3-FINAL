...
In our second test, we see that it also takes approximately 10 relaxations. However, we see it takes slightly more, at 12 relaxations.
...
It happens that the total distance of the graph itself here has high variation from data point to data point, because we only generate one random graph per data point. However, it is interesting to see that the peaks of the real total distance correspond to the peaks of the approximation total distance.
...
Our fourth test is named $test6$ in our python file. It behaves similarly to $test5$, but instead of increasing the node count, we increase the variance of the edge weights. The idea of this test is to see if the range of values the edges can have affects the total distance of the approximation.
...
\subsection{Finding the Time Complexity}
 ...
 By inspecting the algorithm, we see that we don't ever loop on the order of the edges in the graph. We do check edges--but only on the order of the number of nodes. In other words, the order of our mystery function is invariant on how many edges the graph has.

Thus, to derive the time complexity of our mystery function, we are going to run it on varying sized graphs.

Our data points are to check are graph sizes 0 to 500, skipping every 50.

Then, we generate an empty weight graph with $n$ nodes and 0 edges and time the mystery function on this empty graph.

Next we generate a new graph. Here, we completely fill the graph with edges that contain a random edge weight, ranging from -$500$ to $500$. This would be considered a graph's "worst case", where there are a lot of edges to traverse. We also show that this function works on negative edge weights.

We repeat this process $k$ times, (here $k$ = 5) so that we average the time results (because other processes may affect run-time on the CPU, so we want to reduce this as a variable).

Having both an empty graph and a filled graph gets us both our best and worst cases, so we can see how this would affect the time complexity of the function.

We also plot the line of $x^3$ and $x^2$ on our log/log graph. This is an intuition based on how the code works, but it is only a benchmark. They will act as a reference to determine the complexity of the mystery algorithm.
...
\subsubsection{How the algorithm works}
    \begin{itemize}
        \item The algorithm is provided all of the same input as Dijkstra, with an additional input that is referred to as a heuristic function
        ...
        \item With this new heuristic function, the nodes in the queue are now ordered by their distance from the source plus the numerical value the heuristic function returns (for that specific node). 
...
\begin{enumerate}
    \item What issues with Dijkstra’s algorithm is A* trying to address?\\
        The biggest problem with Dijkstra’s algorithm is that it considers all possible paths between nodes, which can lead the algorithm down unideal paths. Meaning that runtime may be wasted on paths unlikely to lead to the shortest path. 
    ...
    \item If you generated an arbitrary heuristic function (similar to randomly generating weights), how would Dijkstra’s algorithm compare to A*?\\
        In that case, they may perform similarly, or more likely, Dijkstra’s may outperform A*. This is because this arbitrary heuristic function could lead the algorithm down even more unideal paths. On the other hand, Dijkstra would be more thorough and would consider all possible paths between nodes. A better heuristic function would estimate the distance between nodes and the destination node. 
...
To get around this roadblock, python users instead implement interfaces as abstract classes.
...
% “adapter”.
The reason the A\_star class does not exactly line up with the one from part two is because the function in part two took in an additional input that was the heuristic function (implemented as a dictionary). 
To get around this while complying with the open close principal we used the original function from part two as an auxiliary function that the function $calc\_sp()$ (given in the UML) uses. This auxiliary function remains unchanged, except for a rename, as we now call it $calc\_sp\_aux()$. 
...
This design pattern complies with the open-close principle. This is because by doing so, 
...
To make our application even more robust (in terms of accepting graphs with nodes represented by non-integer values), we would have to create new functions that encapsulate the way nodes are represented. Right now, we treat the values of the nodes as node identifiers. We should separate these parts of the node to make the functionality more robust. This is why we can't and shouldn't access the nodes by iteration over the number of them -- we make a major assumption that the node values are the keys, and that is something we really shouldn't be assuming.
To alter the design, we should make a separate Node abstract class and have the 
nodes in the graph be of type Node instead. Within the Node class, we would use generics to have a variable node value. 
This way, the node can have any type. In the case where the nodes store more than one value all that would need to be done is extend the node class with a new class that contains the other values that must also be stored. 
...
Currently, the only graph we have is a Heuristic Graph, and an directed weighted graph. However, there are more types of graphs we can make implement our "Graph" class.
...
\subsection{Part 2}
What we can determine from Part 2 is that
\begin{itemize}
    \item A* is a variation of Dijkstra’s Algorithm which takes into account a heuristic function when ordering nodes on their distance to the destination. 
    \item This heuristic function is used to guide the algorithm away from potentially bad paths
    \item This helps to decrease the time wasted on paths unlikely to lead to the shortest paths
\end{itemize}

\subsection{Part 3}

A* gerenally performs much better than Dijkstra’s algorithm, as it performs approximately twice as fast as Dijkstra’s algorithm. This is because A* uses a heuristic function to compute the real time distance between the current node and the goal node. This allows A* to skip over nodes that are not on the shortest path, and thus reducing the number of nodes that need to be visited.\\
\\
...
\subsection{Part 4}
After refactoring our application, our design improves in terms of robustness and makes it more anticipated for change. 
In this new version, we can interchange between various graphs and shortest path algorithms, with the option to add more graph classes and shortest path algorithms in the future. 
However, at the moment our application only accomodates for nodes that store integer values. So a better refactoring would account for any node type (including nodes with multiple values). 
